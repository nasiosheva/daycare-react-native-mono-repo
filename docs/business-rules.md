...
- Pemilih **Tetapkan Goal** selalu dimulai dengan Program Kurikulum aktif yang tersedia untuk tenant. Setelah memilihnya, UI dan API hanya menerima Program Perkembangan aktif yang memang ditautkan ke Program Kurikulum tersebut. Program Perkembangan tenant tetap harus sesuai dengan `LearningLevel` kelas aktif anak; Program global tetap harus sesuai rentang usia anak bila rentang usia tersedia.
...
- Catatan perkembangan memakai kategori bawaan maupun kategori tenant yang aktif. Staff hanya dapat menulis dalam scope anaknya. Setiap catatan pada satu kategori dapat memiliki tepat nol atau satu foto bukti opsional; Staff memilih foto dari galeri atau mengambilnya dengan kamera. Foto hanya menerima JPEG/PNG sampai 5 MB, disimpan bersama catatan, dan tidak dikirim pada respons daftar riwayat.
- Staff Admin, Staff dalam scope anak, dan Parent yang terhubung dapat membaca foto melalui catatan perkembangan anak yang sama. Otorisasi foto wajib memakai tenant dan scope anak yang sama seperti daftar catatan; foto tidak dapat dibaca hanya dengan mengetahui ID catatan.
...
- Profil keluarga Parent adalah data sensitif global: hanya pemilik akun `registrationRole=PARENT` yang dapat membaca atau mengubahnya. Data tidak tersedia bagi Staff, Staff Admin, Platform Admin, maupun endpoint tenant dan tidak boleh dipakai untuk kandidat/template global.
