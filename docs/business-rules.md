...
- **Platform Admin (`ADMIN`)** mengelola lifecycle tenant, langganan, pembayaran tenant, master jenis lembaga, kurikulum global, Program Perkembangan global, serta kategori perkembangan global. Pada setiap jenis lembaga, Platform Admin menetapkan secara terpisah apakah Staff Admin tenant boleh melihat jenis pekerjaan Parent dan/atau rentang penghasilan Parent. Home Admin juga menyediakan dashboard kesiapan tenant yang bersifat informatif: ia melaporkan langganan, Staff Admin, cabang aktif, dan sedikitnya satu offering `PUBLISHED`. Tenant baru tidak siap hanya karena jenis lembaga telah dipilih: Staff Admin tetap harus membuat lalu mempublikasikan offering pada cabang yang benar, agar capability dan mode penerimaan tidak teraktifkan tanpa keputusan operasional sadar. Kelas legacy diperiksa hanya bila ada offering `PUBLISHED` yang memerlukan layanan Daycare atau kurikulum akademik. Untuk Daycare, paket layanan dan instruksi pembayaran diperiksa pada tenant, sedangkan kapasitas serta jam operasional diperiksa hanya pada setiap cabang **aktif** yang memiliki offering `PUBLISHED` ber-capability `DAYCARE_OPERATIONS`; cabang nonaktif atau cabang lain tidak boleh membuat tenant tampak belum siap. Dashboard ini tidak mengubah status tenant atau visibilitas katalog Parent.
- **Staff Admin (`STAFF_ADMIN`)** mengelola konfigurasi dan operasi tenant: cabang, anak, kelas, pendamping, Goal, layanan, les privat, pembayaran Parent, booking, dan persetujuan enrollment. Home Staff Admin secara read-only menampilkan kesiapan tenant aktif dengan aturan yang sama seperti dashboard Platform Admin; kartu hanya menunjukkan konfigurasi yang belum lengkap dan membuka hub Kelola, tanpa mengubah akses, langganan, atau visibilitas katalog Parent. Jika belum ada offering `PUBLISHED`, checklist mengarahkan ke **Jenis lembaga/Penawaran** untuk membuat `DRAFT` dan mempublikasikannya; sistem tidak boleh membuat atau mempublikasikan offering otomatis. Hub Kelola menampilkan checklist langkah yang dapat ditindaklanjuti dari respons server; langganan tetap hanya berupa pemberitahuan karena dikelola Platform Admin, dan penambahan anak bukan prasyarat agar lembaga siap menerima pendaftaran.
- Pada Android/iOS, seluruh role dapat menarik layar **Home** ke bawah untuk memuat ulang query Home yang sedang aktif dalam scope mereka. Pull-to-refresh hanya melakukan baca ulang; ia tidak boleh mengubah status, membuat resource, mengirim keputusan, atau me-refresh query/tenant yang tidak sedang aktif. Web tidak menampilkan gesture ini.
...
- Platform Admin membuat tenant melalui wizard halaman penuh empat langkah: **Data lembaga** (nama lembaga, cabang utama, dan satu atau lebih jenis lembaga), **Akun Staff Admin** (nama, username global opsional, email, dan password), **Paket dan langganan** (paket serta trial 1–12 bulan atau harga bulanan tanpa trial), lalu **Tinjau dan buat tenant**. Setiap langkah divalidasi sebelum dapat dilanjutkan dan navigasi kembali mempertahankan draf. Ringkasan akhir tidak pernah menampilkan password. Tenant, cabang utama, subscription/payment awal, aktivitas kurikulum bawaan, serta akun `primaryStaffAdmin` tetap dibuat secara atomik melalui satu submit terakhir; berpindah langkah tidak membuat data server. Kegagalan submit mempertahankan seluruh draf dan tidak diulang otomatis ketika status jaringan tidak pasti karena request sebelumnya mungkin sudah selesai di server.
...
1. Parent mendaftar secara global sebagai `PARENT`, lalu memilih tenant, cabang, anak, dan paket layanan. Katalog tenant yang ditampilkan ke Parent memuat **semua tenant dengan langganan `ACTIVE` atau `TRIAL` dan capability `DAYCARE_OPERATIONS`**, terlepas dari apakah tenant tersebut sudah punya cabang atau Paket Layanan aktif. Tenant `PENDING_PAYMENT`, `SUSPENDED`, atau `EXPIRED` tidak ditampilkan. Tenant tanpa cabang/paket aktif tetap tampil di katalog tetapi Parent belum dapat menyelesaikan pemilihan cabang/paket sampai Staff Admin tenant melengkapi datanya. Katalog dapat difilter berdasarkan nama tenant lewat pencarian server-side (`GET /parent-enrollment/catalog?search=`). UI menampilkan cabang setiap tenant sebagai item tersendiri di bawah nama tenant (struktur tree, bukan daftar tenant datar); menekan satu cabang langsung memilih tenant dan cabang itu sekaligus serta membuka form pengajuan, tanpa langkah pemilihan cabang terpisah di dalamnya. Setiap item cabang menampilkan alamat lengkapnya dan, bila `googleMapsUrl` cabang terisi, tautan yang membuka Google Maps di luar aplikasi.
   Form pengajuan adalah wizard halaman penuh tiga langkah: pilih lembaga/cabang, lengkapi data anak, lalu pilih paket dan tinjau ringkasan. Form panjang tidak ditempatkan di Bottom Sheet. Navigasi Kembali antar-langkah mempertahankan draf; bila Parent mengganti lembaga, pilihan paket lama dibersihkan karena paket dimiliki lembaga, sedangkan data anak tetap dipertahankan. Satu pengajuan menerima 1–10 anak dan satu paket yang sama untuk seluruh anak. Setiap anak wajib memiliki nama depan, gender Male/Female, dan tanggal lahir valid yang tidak berada di masa depan; nama belakang opsional. UI wajib menampilkan validasi di field/section terkait, membatasi tombol Tambah Anak setelah 10 anak, dan tidak mengandalkan error submit generik sebagai validasi pertama.
   Langkah terakhir menampilkan nama lembaga, cabang, seluruh anak, dan detail paket sebelum submit. Harga katalog diberi label sebagai harga **per anak** dan tidak dijumlahkan sebagai total final di client karena server mengunci snapshot harga ketika checkout. UI juga menjelaskan bahwa submit membuat application `PENDING_APPROVAL` per anak, belum membuat invoice, dan pembayaran baru tersedia setelah approval Staff Admin. Kegagalan memuat katalog menyediakan aksi coba lagi; kegagalan submit mempertahankan seluruh draf agar Parent dapat memperbaiki atau mencoba kembali secara sadar.
...
Parent yang memiliki anak **aktif** di satu tenant dapat mengajukan pindah anak itu ke tenant lain dari Profil Anak (`POST /parent-enrollment/transfer`). Alur ini memakai wizard pengajuan yang sama seperti pendaftaran baru (pilih lembaga/cabang tujuan lalu paket), hanya melewati langkah data anak karena nama, gender, dan tanggal lahir disalin langsung dari anak asal; anak tidak boleh sudah aktif di tenant tujuan, dan tidak boleh ada pengajuan pindah lain yang masih `PENDING_APPROVAL` untuk anak yang sama. Pengajuan pindah menghasilkan `Child` baru yang terpisah di tenant tujuan dengan status `PENDING`, disetujui atau ditolak oleh Staff Admin tenant tujuan sama seperti pendaftaran biasa. **Riwayat anak (kehadiran, catatan perkembangan, kesehatan, insiden, Program Pendampingan) tidak pernah ikut pindah** — riwayat itu tetap tersimpan di tenant asal sesuai aturan retensi §9, dan tidak ada relasi query lintas tenant antara kedua record `Child` tersebut selain penanda `transferredFromChildId` pada pengajuan itu sendiri. Anak di tenant asal baru dinonaktifkan (`active=false`, status `TRANSFERRED`) **saat** pengajuan pindah disetujui, bukan saat diajukan; bila pengajuan ditolak atau masih tertunda, anak tetap aktif seperti biasa di tenant asal. Tenant asal tidak perlu menyetujui kepergian anak secara terpisah.
...
- Staff Admin dapat mengelola **Template Program Pendampingan** tenant (`ChildProgramTemplate`): nama, deskripsi, dan daftar Langkah standar (judul, instruksi, panduan rumah). Template murni alat bantu pengisian cepat — menerapkan template ke seorang anak menyalin nama/deskripsi/langkah menjadi Program dan Langkah baru milik anak itu (status `ACTIVE`, tidak dibagikan ke Parent secara default, sama seperti pembuatan manual); tidak ada tautan berkelanjutan antara template dan Program hasil salinannya, sehingga mengubah atau menghapus template tidak memengaruhi Program yang sudah pernah dibuat darinya. Staff dengan permission `canManageChildPrograms` dapat memakai template untuk anak dalam scope-nya, tetapi hanya Staff Admin yang dapat membuat, mengubah, atau menghapus template itu sendiri.
- Perubahan notifikasi terkait Program Pendampingan: membagikan Program atau Langkah ke Parent (`parentVisible` berubah dari `false` ke `true`) mengirim notifikasi inbox/push ke wali yang terhubung; Parent mengirim Umpan Balik memberi notifikasi ke staf yang ditugaskan pada anak tersebut (atau seluruh Staff Admin aktif bila belum ada penugasan langsung). Langkah yang masih belum selesai setelah 7 hari sejak terakhir diperbarui memicu satu notifikasi pengingat terjadwal ke staf yang sama; ini bukan bagian dari fitur Pengingat Staff (yang bersifat jadwal berulang buatan Staff sendiri), melainkan job harian terpisah yang berjalan sekali per Langkah saat pertama kali melewati ambang 7 hari.
...
- Lampiran foto pada catatan perkembangan baru dapat dibuka dalam ukuran penuh (BottomSheet), sama seperti foto tunggal lama, bukan hanya thumbnail kecil yang tidak bisa diperbesar. Lampiran audio bersifat saling eksklusif dalam satu riwayat anak: memutar audio pada satu catatan otomatis menjeda audio catatan lain pada riwayat yang sama yang masih berjalan.
...
- **Catatan kesehatan** (`ChildHealthRecord`) adalah profil tunggal per anak (bukan log berjalan seperti catatan perkembangan) — golongan darah, alergi, kondisi medis, obat-obatan, dan instruksi darurat, semua opsional. Pada flow saat ini, Staff Admin dan Staff dalam **child scope** (penugasan langsung atau melalui rombel aktif, sesuai `ChildScopeService.requireStaffManagedChild`) dapat mengisi/mengubahnya; Parent yang terhubung sebagai wali hanya bisa membaca. Setiap perubahan tercatat di audit log karena data ini sensitif, dan langsung menotifikasi seluruh wali anak yang terhubung (inbox, realtime `HEALTH`, dan push bila perangkat penerima terdaftar) supaya perubahan alergi/kondisi medis/obat tidak luput dari perhatian Parent. Model target least-privilege, medication order, dan log pemberian obat diatur pada §13.13.
