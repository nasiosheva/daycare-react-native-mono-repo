...
ensure_java_21() {
  ...
  [ "$java_version" = "21" ] || fail "JDK 21 is required to build the Android release APK."
}
...
require_release_signing() {
  ...
  grep -Fq 'signingConfig signingConfigs.release' "$android_root/app/build.gradle" || fail "Release signing is not configured in $android_root/app/build.gradle."
}
...
[ -s "$apk_path" ] || fail "Gradle completed without producing $apk_path."
verify_release_signature
