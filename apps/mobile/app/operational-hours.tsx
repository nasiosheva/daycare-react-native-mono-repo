...
export default function OperationalHoursScreen() {
  ...
  return <AppScreen title={t("overtime.parentTitle")}><AppText tone="muted">{t("overtime.parentDescription")}</AppText>
    {hours.isFetching ? <ShimmerList /> : hours.data?.map((entry) => <View key={entry.childId} style={styles.card}><AppText variant="heading">{entry.childName}</AppText><AppText tone="muted">{t("overtime.childTenantLabel", { organization: entry.organizationName, branch: entry.branchName })}</AppText>
      {entry.hours.map((hour) => <View key={hour.dayOfWeek} style={styles.row}><AppText>{t(`overtime.day.${hour.dayOfWeek}`)}</AppText><AppText tone="muted">{hour.active ? `${hour.opensAt}–${hour.closesAt}` : t("overtime.closed")}</AppText></View>)}
      <AppText variant="h5">{t("overtime.rateTiers")}</AppText>{entry.tiers.map((tier, index) => <AppText key={`${tier.durationMinutes}-${index}`} tone="muted">{t("overtime.parentTier", { duration: tier.durationMinutes, amount: formatCurrency(tier.amount) })}</AppText>)}
    </View>)}
