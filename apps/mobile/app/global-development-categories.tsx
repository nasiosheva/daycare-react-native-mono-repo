...
export default function GlobalDevelopmentCategoriesScreen() {
  const router = useRouter();
  const { api, profile } = useAuth();
  const { t } = useI18n();
  const queryClient = useQueryClient();
  const categories = useQuery({ queryKey: ["global-development-categories"], queryFn: () => api.globalDevelopmentCategories(), enabled: Boolean(profile?.isPlatformAdmin) });
  const refresh = () => void queryClient.invalidateQueries({ queryKey: ["global-development-categories"] });
  const createCategory = useMutation({ mutationFn: api.createGlobalDevelopmentCategory.bind(api), onSuccess: refresh });
  const renameCategory = useMutation({ mutationFn: ({ id, name }: { id: string; name: string }) => api.updateGlobalDevelopmentCategory(id, { name }), onSuccess: refresh });
  const updateCategory = useMutation({ mutationFn: ({ id, active }: { id: string; active: boolean }) => api.updateGlobalDevelopmentCategory(id, { active }), onSuccess: refresh });
  const deleteCategory = useMutation({ mutationFn: api.deleteGlobalDevelopmentCategory.bind(api), onSuccess: refresh });
