...
export default function ParentEnrollmentScreen() {
  ...
  const checkout = useMutation({ mutationFn: () => {
    if (!tenant || !branch || !plan || children.some((child) => !child.firstName.trim() || !isIsoDate(child.dateOfBirth))) throw new Error(t("children.required"));
