...
export default function ParentPaymentsScreen() {
  ...
  const pendingInvoices = invoices.data?.filter((invoice) => invoice.status === "PENDING" || invoice.status === "PAYMENT_SUBMITTED") ?? [];
  ...
  return <AppScreen showBottomNavigation={false} title={t("staffAdmin.paymentsTitle")} header={<BackButton accessibilityLabel={t("common.back")} onPress={() => router.back()} />}>
    ...
    {pendingInvoices.map((invoice) => <View key={invoice.id} style={styles.card}>
      ...
      {invoice.status === "PAYMENT_SUBMITTED" ? <><AppText tone="muted">{t("paymentProof.awaitingReview")}</AppText>{canManage && <Button loading={review.isPending} onPress={() => setReviewInvoiceId(invoice.id)}>{t("paymentProof.review")}</Button>}</> : canManage && <Button loading={markPaid.isPending} onPress={() => void pay(invoice.id)}>{t("billing.markPaid")}</Button>}
    </View>)}
