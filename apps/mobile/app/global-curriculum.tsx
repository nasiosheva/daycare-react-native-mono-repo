...
export default function GlobalCurriculumScreen() {
  ...
  const [editing, setEditing] = useState<GlobalCurriculumProgram>();
  const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [learningLevelId, setLearningLevelId] = useState<string>(); const [goalSearch, setGoalSearch] = useState(""); const [debouncedGoalSearch, setDebouncedGoalSearch] = useState(""); const [developmentProgramIds, setDevelopmentProgramIds] = useState<string[]>([]);
  ...
  const closeSheet = () => { setSheetOpen(false); setEditing(undefined); setName(""); setDescription(""); setLearningLevelId(undefined); setGoalSearch(""); setDevelopmentProgramIds([]); };
  ...
  const selectLearningLevel = (nextLearningLevelId: string) => {
    setLearningLevelId(nextLearningLevelId);
    if (!goals.data) return;
    const visibleGoalIds = new Set(goals.data.filter((goal) => goal.learningLevelId === nextLearningLevelId).map((goal) => goal.id));
    setDevelopmentProgramIds((ids) => ids.filter((id) => visibleGoalIds.has(id)));
  };
...
function ProgramCard({ program, learningLevelName, t, onEdit, onActiveChange }: { program: GlobalCurriculumProgram; learningLevelName?: string; t: ReturnType<typeof useI18n>["t"]; onEdit: () => void; onActiveChange: (active: boolean) => void }) {
  return <View style={[styles.card, !program.active && styles.archivedCard]}><AppText variant="label">{program.name}</AppText>{program.description && <AppText tone="muted">{program.description}</AppText>}<AppText variant="caption" tone="muted">{learningLevelName ?? t("common.noData")} · {t("academic.programGoals", { count: program.developmentProgramIds.length })}</AppText><View style={styles.actions}><Button variant="secondary" onPress={onEdit}>{t("common.edit")}</Button><Button variant="secondary" onPress={() => onActiveChange(!program.active)}>{t(program.active ? "learning.archive" : "learning.active")}</Button></View></View>;
}
