...
export default function ParentEnrollmentFormScreen() {
  ...
  return <AppScreen showBottomNavigation={false} title={t("parentEnrollment.newTenant")} header={<BackButton accessibilityLabel={t("common.back")} onPress={() => router.back()} />}><View style={styles.content}>
    ...
    {catalog.isLoading && <AppText tone="muted">{t("common.loading")}</AppText>}
    ...
    {tenant && <><AppText variant="heading">{t("parentEnrollment.branch")}</AppText>{tenant.branches.map((item) => <Button key={item.id} variant={branch?.id === item.id ? "primary" : "secondary"} onPress={() => setBranchId(item.id)}>{item.name}</Button>)}
      ...
      <Button loading={checkout.isPending} disabled={!tenant || !branch || !plan} onPress={() => checkout.mutate()}>{t("parentEnrollment.submitApplication")}</Button>
    </>}
  </View></AppScreen>;
}
