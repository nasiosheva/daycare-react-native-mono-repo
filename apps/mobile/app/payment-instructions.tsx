...
export default function PaymentInstructionsScreen() {
  ...
  const [isFormOpen, setIsFormOpen] = useState(false); const [editing, setEditing] = useState<PaymentInstruction | null>(null); const [pendingDelete, setPendingDelete] = useState<PaymentInstruction | null>(null); const [draft, setDraft] = useState<UpsertPaymentInstructionInput>(emptyDraft); const [error, setError] = useState<string | null>(null);
  const instructions = useQuery({ queryKey: ["payment-instructions", organizationId, "manage"], queryFn: () => api.managedPaymentInstructions(), enabled: membership?.role === "STAFF_ADMIN" });
  const close = () => { setIsFormOpen(false); setEditing(null); setDraft(emptyDraft); setError(null); };
  const save = useMutation({ mutationFn: () => editing ? api.updatePaymentInstruction(editing.id, draft) : api.createPaymentInstruction(draft), onSuccess: () => { void client.invalidateQueries({ queryKey: ["payment-instructions", organizationId] }); close(); }, onError: (value) => setError(value instanceof Error ? value.message : t("paymentInstruction.saveFailed")) });
  const remove = useMutation({ mutationFn: (id: string) => api.deletePaymentInstruction(id), onSuccess: () => { void client.invalidateQueries({ queryKey: ["payment-instructions", organizationId] }); setPendingDelete(null); }, onError: (value) => setError(value instanceof Error ? value.message : t("paymentInstruction.saveFailed")) });
  const openForCreate = () => { setEditing(null); setDraft(emptyDraft); setError(null); setIsFormOpen(true); };
  const openForEdit = (instruction: PaymentInstruction) => { setEditing(instruction); setDraft({ name: instruction.name, accountHolder: instruction.accountHolder, accountNumber: instruction.accountNumber, note: instruction.note ?? "", active: instruction.active, displayOrder: instruction.displayOrder }); setError(null); setIsFormOpen(true); };
  const submit = () => {
    if (!draft.name.trim() || !draft.accountHolder.trim() || !draft.accountNumber.trim()) { setError(t("paymentInstruction.requiredFields")); return; }
    save.mutate();
  };
