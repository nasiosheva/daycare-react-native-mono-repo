...
export default function PickupAuthorizationsScreen() {
  ...
  const authorizations = useQuery({ queryKey: ["pickup-authorizations", organizationId, childId], queryFn: () => api.pickupAuthorizations(childId!), enabled: Boolean(childId && (membership?.role === "PARENT" || membership?.role === "STAFF_ADMIN")) });
  ...
  if (!childId || !["PARENT", "STAFF_ADMIN"].includes(membership?.role ?? "")) return <Redirect href="/home" />;
  const submit = async () => {
  ...
  return <AppScreen showBottomNavigation={false} title={t("pickup.manage")} header={<BackButton accessibilityLabel={t("common.back")} onPress={() => router.back()} />}><View style={styles.content}>
    {membership?.role === "PARENT" && <Button onPress={() => setOpen(true)}>{t("pickup.add")}</Button>}
