...
type StaffAdminSummaryInput = {
  ...
  pendingEnrollments?: readonly unknown[];
...
export function createStaffAdminSummary({ children, users, pendingBookings, pendingEnrollments = [], invoices, entitlements }: StaffAdminSummaryInput): StaffAdminSummary {
  ...
  return {
    ...
    pendingApprovals: pendingBookings.filter((booking) => booking.status === "PENDING_APPROVAL").length + pendingEnrollments.length,
