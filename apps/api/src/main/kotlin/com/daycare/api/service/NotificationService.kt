...
class NotificationService(
    ...
    fun notify(organizationId: UUID, recipientUserId: UUID, title: String, body: String) {
        notifications.save(Notification(organizationId = organizationId, recipientUserId = recipientUserId, title = title, body = body))
        deviceTokens.findAllByUserIdAndOrganizationId(recipientUserId, organizationId).forEach { token ->
            runCatching { restClient.post().uri(expoPushUrl).body(mapOf("to" to token.token, "title" to title, "body" to body, "sound" to "default")).retrieve().toBodilessEntity() }
