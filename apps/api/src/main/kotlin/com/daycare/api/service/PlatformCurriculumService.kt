package com.daycare.api.service

import com.daycare.api.persistence.CurriculumProgram
import com.daycare.api.persistence.CurriculumProgramDevelopmentProgram
import com.daycare.api.persistence.CurriculumProgramDevelopmentProgramRepository
import com.daycare.api.persistence.CurriculumProgramRepository
import com.daycare.api.persistence.DevelopmentProgramRepository
import com.daycare.api.persistence.LearningLevelCurriculumProgram
import com.daycare.api.persistence.LearningLevelCurriculumProgramRepository
import com.daycare.api.persistence.LearningLevelRepository
import jakarta.validation.constraints.NotBlank
import jakarta.validation.constraints.Size
import org.springframework.security.oauth2.jwt.Jwt
import org.springframework.stereotype.Service
import org.springframework.transaction.annotation.Transactional

data class CreateGlobalCurriculumProgramRequest(
    val learningLevelId: java.util.UUID,
    @field:NotBlank @field:Size(max = 120) val name: String,
    @field:Size(max = 2_000) val description: String = "",
    val developmentProgramIds: Set<java.util.UUID> = emptySet(),
)
data class GlobalCurriculumProgramResponse(
    val id: java.util.UUID,
    val learningLevelId: java.util.UUID?,
    val name: String,
    val description: String,
    val source: CurriculumProgramSource,
    val isTemplate: Boolean,
    val active: Boolean,
    val developmentProgramIds: Set<java.util.UUID>,
)

@Service
class PlatformCurriculumService(
    private val platformAccess: PlatformAccessService,
    private val programs: CurriculumProgramRepository,
    private val programGoals: CurriculumProgramDevelopmentProgramRepository,
    private val developmentPrograms: DevelopmentProgramRepository,
    private val levels: LearningLevelRepository,
    private val levelPrograms: LearningLevelCurriculumProgramRepository,
    private val globalCurriculumSeeding: GlobalCurriculumSeedingService,
) {
    @Transactional
    fun seedGlobalCurriculum(jwt: Jwt): GlobalCurriculumSeedResult {
        platformAccess.requirePlatformAdmin(jwt)
        return globalCurriculumSeeding.seed()
    }

    @Transactional(readOnly = true)
    fun programs(jwt: Jwt, includeArchived: Boolean = false): List<GlobalCurriculumProgramResponse> {
        platformAccess.requirePlatformAdmin(jwt)
        return (if (includeArchived) programs.findAllByOrganizationIdIsNullOrderByNameAsc() else programs.findAllByOrganizationIdIsNullAndActiveTrueOrderByNameAsc()).map(::response)
    }

    @Transactional
    fun createProgram(jwt: Jwt, request: CreateGlobalCurriculumProgramRequest): GlobalCurriculumProgramResponse {
        platformAccess.requirePlatformAdmin(jwt)
        val learningLevel = globalLearningLevel(request.learningLevelId)
        validateGoals(request.developmentProgramIds, learningLevel.id, existingProgramId = null)
        val program = programs.save(CurriculumProgram(name = request.name.trim(), description = request.description.trim(), isTemplate = true))
        replaceReferenceLearningLevel(program.id, learningLevel.id)
        replaceGoals(program.id, request.developmentProgramIds)
        return response(program)
    }

    @Transactional
    fun updateProgram(jwt: Jwt, programId: java.util.UUID, request: CreateGlobalCurriculumProgramRequest): GlobalCurriculumProgramResponse {
        platformAccess.requirePlatformAdmin(jwt)
        val learningLevel = globalLearningLevel(request.learningLevelId)
        validateGoals(request.developmentProgramIds, learningLevel.id, existingProgramId = programId)
        val program = globalProgram(programId)
        program.name = request.name.trim()
        program.description = request.description.trim()
        replaceReferenceLearningLevel(program.id, learningLevel.id)
        replaceGoals(program.id, request.developmentProgramIds)
        return response(program)
    }

    @Transactional
    fun setProgramActive(jwt: Jwt, programId: java.util.UUID, active: Boolean): GlobalCurriculumProgramResponse {
        platformAccess.requirePlatformAdmin(jwt)
        return response(globalProgram(programId).also { it.active = active })
    }

    private fun validateGoals(ids: Set<java.util.UUID>, learningLevelId: java.util.UUID, existingProgramId: java.util.UUID?) = ids.forEach { id ->
        val program = developmentPrograms.findById(id).orElseThrow { IllegalArgumentException("Development program was not found") }
        require(program.organizationId == null) { "Development program is not available for a global curriculum program" }
        require(program.learningLevelId == learningLevelId) { "Development program belongs to a different learning level" }
        require(program.active || (existingProgramId != null && programGoals.existsByCurriculumProgramIdAndDevelopmentProgramId(existingProgramId, id))) { "Archived Goal template cannot be assigned to a new curriculum program" }
    }
    private fun replaceGoals(programId: java.util.UUID, ids: Set<java.util.UUID>) { programGoals.deleteAllByCurriculumProgramId(programId); programGoals.saveAll(ids.map { CurriculumProgramDevelopmentProgram(curriculumProgramId = programId, developmentProgramId = it) }) }
    private fun replaceReferenceLearningLevel(programId: java.util.UUID, learningLevelId: java.util.UUID) {
        val globalLevelLinks = levelPrograms.findAllByCurriculumProgramId(programId).filter { link ->
            levels.findById(link.learningLevelId).orElse(null)?.organizationId == null
        }
        levelPrograms.deleteAll(globalLevelLinks)
        levelPrograms.save(LearningLevelCurriculumProgram(learningLevelId = learningLevelId, curriculumProgramId = programId))
    }
    private fun globalProgram(programId: java.util.UUID) = programs.findById(programId).orElseThrow { IllegalArgumentException("Curriculum program was not found") }.also { require(it.organizationId == null) { "This curriculum program is not global" } }
    private fun globalLearningLevel(learningLevelId: java.util.UUID) = levels.findById(learningLevelId).orElseThrow { IllegalArgumentException("Global learning level was not found") }.also { level -> require(level.organizationId == null && level.active) { "Learning level is not available for a global curriculum program" } }
    private fun referenceLearningLevelId(programId: java.util.UUID) = levelPrograms.findAllByCurriculumProgramId(programId)
        .firstOrNull { link -> levels.findById(link.learningLevelId).orElse(null)?.organizationId == null }
        ?.learningLevelId
    private fun response(program: CurriculumProgram) = GlobalCurriculumProgramResponse(program.id, referenceLearningLevelId(program.id), program.name, program.description, CurriculumProgramSource.GLOBAL, program.isTemplate, program.active, programGoals.findAllByCurriculumProgramId(program.id).map { it.developmentProgramId }.toSet())
}
