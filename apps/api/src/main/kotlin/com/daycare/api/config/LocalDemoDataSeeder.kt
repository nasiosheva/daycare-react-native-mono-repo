...
class LocalDemoDataSeeder(
    ...
    override fun run(args: ApplicationArguments) {
        ...
        parentEnrollments.saveAll(listOf(ParentEnrollment(id = id("enrollment-bima"), userId = applicant.id, organizationId = tenant, branchId = branch, childId = bima.id, invoiceId = invoiceBima.id, entitlementId = entitlementBima.id, status = ParentEnrollmentStatus.PENDING_APPROVAL), ParentEnrollment(id = id("enrollment-damar"), userId = applicant.id, organizationId = tenant, branchId = branch, childId = damar.id, invoiceId = invoiceDamar.id, entitlementId = entitlementDamar.id, status = ParentEnrollmentStatus.PENDING_PAYMENT)))
        ...
        development.save(DevelopmentEntry(id = id("development-aruna"), organizationId = tenant, branchId = branch, childId = aruna.id, authorUserId = teacher.id, category = DevelopmentCategory.ACTIVITY, title = "Kolase warna", content = "Aruna mengikuti aktivitas kolase dengan antusias.", recordedAt = now.minusSeconds(3_600)))
