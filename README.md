...
- Firebase email/password and Google authentication; Firebase ID tokens secure the API. The Google sign-in button is shown whenever Firebase auth is active (`EXPO_PUBLIC_LOCAL_AUTH_ENABLED` is not `true`), including the production build. The sign-in screen no longer offers a phone-number entry point. The API resolves a configured username to its required account email before Firebase performs Email/Password authentication.
