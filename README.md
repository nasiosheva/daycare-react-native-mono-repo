...
- Platform Admin Home also provides a read-only **Tenant readiness** card. It groups ready tenants and tenants needing attention for subscription, active Staff Admin, active branch, and at least one explicitly published institution offering; a legacy class group is required only where a published Daycare or `ACADEMIC_CURRICULUM` offering is available. A tenant with a published Daycare offering additionally requires an active service plan and payment instruction, while capacity and valid weekly operating hours are required only for each active branch that has that published Daycare offering. It is a support dashboard only: it does not change tenant status or Parent catalog visibility.
...
- Shared mobile UI includes an accessible bottom sheet with a drag handle and close button. Operational add, edit, assignment, and password forms begin from an explicit action instead of rendering immediately; short independent forms open in a Bottom Sheet. Long dependent workflows—especially enrollment, checkout, billing, approval, or account creation with multiple sections or a final review—use a dedicated full-page multi-step form wizard with a visible stepper, per-step validation, draft-preserving back navigation, and a clear final summary. Their common shell is `MultiStepFormWizard` from `packages/ui`: it accepts one or more dynamic steps, keeps the active index controlled by the feature screen, and horizontally scrolls the indicator when many steps cannot fit safely. Connector lines remain white before their destination step is reached and use the shared primary gold color after it is reached. Profile uses the Bottom Sheet to confirm logout before ending the session.
- On Android and iOS, every role's Home supports pull-to-refresh. The gesture refetches only the active data already used by that role's Home (including readiness, notices, and scoped operational summaries where applicable); it performs no mutation and is intentionally not shown on web.
...
| Local Web + API | — | — | `./scripts/run-web-local-stack.sh` |
...
All API routes are under `/api/v1` and require an application or Firebase bearer token except the OpenAPI/Swagger endpoints and the public application-password registration/login routes. Email/username-and-password login always returns an application bearer token for the same protected API routes. Firebase Google and phone tokens are accepted only to verify and match an existing identity, or while submitting Parent registration; they never create an account automatically. Every self-registration is recorded with the global `PARENT` registration role; this does not grant access to a tenant. A Parent without tenant access uses the onboarding navigation (Home, Enrollment, Profile) and a full-page three-step enrollment wizard to choose a tenant branch, enter one to ten children, and select one package for all of them. The final review identifies the catalog price as a per-child price and explains that submission creates a separate `PENDING_APPROVAL` application with a locked package and price snapshot for every child, but no invoice. After Staff Admin approval, the Parent membership, guardian link, invoice, and pending entitlement are created or activated; the Parent then follows the tenant's transfer instructions and uploads proof. `POST /api/v1/auth/logout` revokes the active bearer token server-side until its natural expiry; only its SHA-256 hash is retained. Mobile logout sends this request best-effort while immediately clearing the local session and returning to Login, so an offline device still exits locally without waiting for a response. Endpoints that operate on an organization also require `X-Organization-Id`. The mobile app supports Indonesian, English, Simplified Chinese, French, Portuguese, Spanish, and Russian; it saves the chosen language on the device and sends it in `Accept-Language`. The API localizes supported error details and uses Indonesian when the header is absent or unsupported. Raw Firebase provider messages and configuration details are not exposed to clients.
...
| Manage tenant-wide child-program templates (Staff Admin) | `GET/POST /api/v1/child-program-templates`, `PATCH/DELETE /api/v1/child-program-templates/{templateId}` |
| Create a child's program from a template | `POST /api/v1/children/{childId}/programs/from-template/{templateId}` |
| Read active-program/feedback counts for Home | `GET /api/v1/children/programs-summary` (Staff Admin), `GET /api/v1/parent/children/programs-summary` (Parent) |
