...
- Firebase email/password, phone, and Google authentication; Firebase ID tokens secure the API. The phone and Google sign-in buttons are hidden in the production build (`EXPO_PUBLIC_APP_ENV=production`); only email/password sign-in is offered there.
...
| `EXPO_PUBLIC_APP_ENV` | Mobile/web | Simulation and production only | Set to `simulation` only in `.env.simulation` to enable the local role-preview buttons. Set to `production` in the production deploy workflow (and `.env.prod`) to hide the Google and phone-number sign-in buttons, which are Firebase-only conveniences not offered in the real production build. |
...
Local startup never creates accounts, passwords, memberships, tenants, or transactional/demo data. Flyway alone provides idempotent master data: built-in institution types, global development categories, and global Goal templates. Create a local user through the intended authentication/provisioning flow before signing in; local database reset therefore starts with master data only.
