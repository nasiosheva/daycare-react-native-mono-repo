...
- Staff Admin may link an existing Parent account to a child by exact username or email, including during direct/offline child registration. Linking creates or reactivates only the tenant Parent membership and guardian relation; it does not create an enrollment application, invoice, entitlement, or booking. The link can later be removed from the child detail screen.
