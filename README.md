...
Android and iOS use a native splash screen configured from `apps/mobile/app.json`. It shows the existing Umur Emas logo on a white background while the application restores its initial authentication state, then fades into the Expo Router application. Web intentionally does not use this native splash behavior. The generated `android` and `ios` directories remain local-only; rerun the relevant mobile launcher after changing the splash configuration or asset so Expo prebuild synchronizes the native projects.
