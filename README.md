...
- Staff Admin Home uses the same readiness rules for its current tenant. When setup is incomplete, it shows a read-only card listing the missing configuration and routes to the Staff Admin management hub; that hub flags each matching configuration card with its outstanding requirement, while an inactive tenant subscription remains a non-actionable notice because only Platform Admin manages it. It does not change access, subscription, or Parent catalog visibility.
