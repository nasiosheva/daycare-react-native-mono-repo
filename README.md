...
Create a protected GitHub Environment named `golden-age` and configure these values there before enabling the first deployment. Public build settings are preferably Environment Variables; the deployment workflow also accepts an Environment Secret with the same name when a value has been stored there instead.
